...
class EGNN(nn.Module):
    ...
    def forward(self, h, pos, edge_index, edge_attr=None):
        ...
        Args:
            h: Node features [num_nodes, in_node_nf]
            pos: Node coordinates [num_nodes, 3]
            edge_index: Edge indices [2, num_edges]
            edge_attr: Edge attributes [num_edges, in_edge_nf] (optional)
        
        Returns:
            pos: Predicted coordinates [num_nodes, 3]
        """
        h = self.embedding_in(h)
        for i in range(0, self.n_layers):
            h, pos, _ = self._modules["gcl_%d" % i](h, edge_index, pos, edge_attr=edge_attr)
